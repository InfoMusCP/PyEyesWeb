...
try:
    ...
    from tests.benchmark.real_data_tester import RealDataTester
except ModuleNotFoundError:
    # When run as script: python tests/feature_test_cli.py
    from test_helpers import FeatureTester, Colors
    from benchmark.real_data_tester import RealDataTester
...
class SmoothnessTester(FeatureTester):
    ...
    def test(self, signal_type: str, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        length, signal2_type = (self._extract_common_params(**kwargs)[0], None)
...
def main():
    ...
    if args.use_real_data:
        ...
        print(f"{Colors.WARNING}Using real data files is not yet implemented in this CLI.{Colors.ENDC}")
        results = RealDataTester(args.feature).run()
        return
