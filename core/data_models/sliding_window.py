...
class SlidingWindow:
    ...
    def __init__(self, max_length: int, n_columns: int):
        ...
        self._buffer = np.zeros((max_length, n_columns), dtype=np.float32)
        self._timestamp = np.zeros(max_length, dtype=np.float64)

        self._start = 0
        self._size = 0

    def append(self, value: np.ndarray | list, timestamp: Optional[float] = None) -> None:
